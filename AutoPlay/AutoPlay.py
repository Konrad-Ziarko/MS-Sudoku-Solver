...
class ADS():
    ...
    def addStream(self,stream,content):
        if os.path.exists("%s:%s" % (self.filename, stream)):
            print ("A Stream with the same name already exist.")
            return False

        fd = open("%s:%s" % (self.filename, stream), "w")
        fd.write(content)
        fd.close()
...
def main():
    file = r"C:\Users\Konrad\Source\Repos\AutoPlay\AutoPlay\AutoPlay\AutoPlay.log"
    handler = ADS(file)
    mystring = "dupa dupa"
    #mystring.encode('utf-8')
    handler.addStream("dupa.txt", mystring)
    print(handler.retrieveStreams())
